...
const ForDeveloperPage = () => {
  ...
  const handleClickSubmit = async () => {
    ...
    try {
      await startNewAgency({ ...formValue, type: 'developer' });
      setIsSuccess(true);
    } catch (e) {
  ...
  const renderTitle = () => {
    return (<div className={styles['for-developer-page__title']}>
      Generate leads for FREE
    </div>)
  };

  const renderHeader = () => {
    return (<div className={styles['for-developer-page__header']}>
      <img src="/images/zeekeez-for-developer.png" />
    </div>)
  };
  ...
  const renderBenefits = () => {
    return (<ul className={styles['for-developer-page__list']}>
      <li><span className={styles['for-developer-page__list-icon']}><CheckIcon /></span>unlimited listings</li>
      <li><span className={styles['for-developer-page__list-icon']}><CheckIcon /></span>unlimited medias</li>
      <li><span className={styles['for-developer-page__list-icon']}><CheckIcon /></span>unlimited users</li>
      <li><span className={styles['for-developer-page__list-icon']}><CheckIcon /></span>no credit card required</li>
      <li><span className={styles['for-developer-page__list-icon']}><CheckIcon /></span>non-binding offer</li>
    </ul>
    );
  };

  const renderJoin = () => {
    return (<p className={styles['for-developer-page__join']}>
      Join zeekeez’s network and build the right relationship with agencies, buyers, sellers and renters in the UAE. Get notified with new matches.
    </p>);
  };
  ...
  const renderTechList = () => {
    return (<ul className={styles['for-developer-page__list']}>
      <li><span className={styles['for-developer-page__list-icon']}><CheckIcon /></span>free unlimited access for early adopters (free for a limited time)</li>
      <li><span className={styles['for-developer-page__list-icon']}><CheckIcon /></span>best technology for an unbeatable user experience (React, NextJS, Elasticsearch...)</li>
      <li><span className={styles['for-developer-page__list-icon']}><CheckIcon /></span>GIS algorithms for the best precision (cities, communities, sub-communities...)</li>
      <li><span className={styles['for-developer-page__list-icon']}><CheckIcon /></span>new UX/UI for an intuitive user journey</li>
      <li><span className={styles['for-developer-page__list-icon']}><CheckIcon /></span>best confidentiality for your data, we are not backed by agencies</li>
    </ul>);
  };

  const renderDoBest = () => {
    return (<p className={styles['for-developer-page__do-best']}>
      Do what you do best, even better, even faster with zeekeez.
    </p>);
  };

  const renderReshape = () => {
    return (<p className={styles['for-developer-page__reshape']}>
      Join us to reshape UAE’s Real Estate market!
    </p>);
  };

  const renderFormTitle = () => {
    return (<div className={styles['for-developer-page__form-title']}>
      Promote your listings now, complete the form
    </div>);
  };

  const renderForm = () => (
    <div className={styles['for-developer-page__form']}>
      <div className={styles['for-developer-page__form__inputs-text']}>
        <MyPZTextField
          label="First Name"
      ...
      <div className={styles['for-developer-page__form__inputs-area']}>
        <InputTextArea
          label="Message"
      ...
      <div className={styles['for-developer-page__form__inputs-button']}>
        {renderAlert()}
